...
@Component({
  selector: 'app-dem-pay-cheque-list-accepte',
  templateUrl: './dem-pay-cheque-list-accepte.component.html',
  styleUrls: ['./dem-pay-cheque-list-accepte.component.css']
})
export class DemPayChequeListAccepteComponent implements OnInit {
  ...
  getDemPayList() {
    this.demandeService.Get().subscribe(res => {
      this.dem5 = res
      this.dem6 = this.dem5.filter(item => item.etatgeneral == "معتمدة")
